...
static void map_host_memory(void)
{
	int i;
	struct mem_section *section;
	...
	for (i = 1; i < nr_sections; i++) {
		section = &mem_sections[i];
		mmu_map_host_memory(section->phy_base, section->phy_base,
...
__dynamic_get_mem_block(struct mem_section *section, unsigned long f)
{
	unsigned long bit;
	struct mem_block *block;
	unsigned long flags;
...
static struct mem_block *dynamic_get_mem_block(unsigned long flags)
{
	int i;
	...
	for (i = 0; i < nr_sections; i++) {
		section = &mem_sections[i];
		block = __dynamic_get_mem_block(section, flags);
		if (block)
			break;
	}
...
again:
	...
	block = dynamic_get_mem_block(GPF_PAGE_META);
	if (!block)
		return NULL;
...
static void *get_pages_from_block(struct mem_block *block, int count, int align)
{
	int bit, i;
	void *addr;
	struct page *meta;
...
static void *dynamic_get_free_pages(int count, int align)
{
	void *base = NULL;
	struct mem_block *block = NULL, *n = NULL;
	unsigned long f;
	unsigned long *page_meta;
	...
	list_for_each_entry_safe(block, n, &page_pool->block_list, list) {
		base = get_pages_from_block(block, count, align);
		if (base) {
			if (block->free_pages == 0)
				list_del(&block->list);
			break;
	...
	block = dynamic_get_mem_block(GFB_PAGE);
	if (!block)
		goto out;
...
static int alloc_new_slab_block(void)
{
	...
	block = dynamic_get_mem_block(GFB_SLAB);
	if (!block)
		return -ENOMEM;
...
static void dynamic_free_mem_block(struct mem_block *block)
{
	unsigned long start;
...
static void dynamic_free_pages(void *addr)
{
	struct mem_block *block;
	unsigned long start;
	unsigned long flags;
	...
	meta = (struct page *)block_meta_base(block);
	count = meta->phy_base & 0xfffffffffffff000;
	block->free_pages += count;
...
static void dynamic_free(void *addr)
{
	struct slab_header *header;
	struct slab_pool *slab_pool;
	unsigned long flags;
...
static int dynamic_mm_init(void)
{
	size_t size;
...
struct mem_allocator dynamic_allocator __used = {
	.name			= "dynamic allocater",
	.init			= dynamic_mm_init,
	.malloc			= dynamic_malloc,
	.get_free_pages_align 	= dynamic_get_free_pages,
	.free			= dynamic_free,
	.free_pages		= dynamic_free_pages,
	.get_mem_block		= dynamic_get_mem_block,
	.free_mem_block 	= dynamic_free_mem_block,
};
